    ...
    return {
      ImportDeclaration(node) {
        ...
        if (comment && !comments.includes(comment.value.trim())) {
          ...
          if (!externalError && comments.filter(comment => comment.includes(aliasToName[alias])).length > 0 && comments[0] !== "Externals") {
            context.report({
              node,
        ...
        if (nameToAlias[comments[comments.length - 1]] !== alias) {

          const message = `@/${alias} should not be in the ${comments} group.`;
          context.report({
            node,
            message,
          });
